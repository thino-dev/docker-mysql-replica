...
# 從 master 獲取複製配置
MASTER_LOG_FILE=$(mysql -h "$MYSQL_HOST_MASTER" -u "$REPL_USER" -p"$REPL_USER_PASSWORD" -e "SHOW BINARY LOG STATUS\G" | grep "File:" | awk '{print $2}')
MASTER_LOG_POS=$(mysql -h "$MYSQL_HOST_MASTER" -u "$REPL_USER" -p"$REPL_USER_PASSWORD" -e "SHOW BINARY LOG STATUS\G" | grep "Position:" | awk '{print $2}')

# 檢查 slave 狀態
SLAVE_STATUS=$(mysql -u root -p"$MYSQL_ROOT_PASSWORD" -e "SHOW REPLICA STATUS\G")
if [ -z "$SLAVE_STATUS" ]; then
    mysql -u root -p"$MYSQL_ROOT_PASSWORD" <<-EOSQL
CHANGE REPLICATION SOURCE TO SOURCE_HOST='$MYSQL_HOST_MASTER', SOURCE_USER='$REPL_USER', SOURCE_PASSWORD='$REPL_USER_PASSWORD', SOURCE_LOG_FILE='$MASTER_LOG_FILE', SOURCE_LOG_POS=$MASTER_LOG_POS;
START REPLICA;
EOSQL
else
    mysql -u root -p"$MYSQL_ROOT_PASSWORD" <<-EOSQL
